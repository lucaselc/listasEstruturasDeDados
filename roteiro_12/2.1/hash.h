...
int insereHash_semTratar_div(Hash* h, int elem){
    if(h == NULL) return 0;
...
int insereHash_semTratar_mul(Hash* h, int elem){
    if(h == NULL) return 0;
...
int insereHash_semTratar_dobra(Hash* h, int elem){
    if(h == NULL) return 0;
    int pos = chaveDobra(elem, h->tam);
...
int buscaHash_semTratar_div(Hash* h, int elem, int *p){
    if(h == NULL) return 0;
    int pos = chaveDivisao(elem, h->tam);
...
int buscaHash_semTratar_mul(Hash* h, int elem, int *p){
    if(h == NULL) return 0;
    int pos = chaveMultiplicacao(elem, h->tam);
    if(h->tabela[pos] == NULL) return 0;
    if(*(h->tabela[pos]) == elem){
...
int buscaHash_semTratar_dobra(Hash* h, int elem, int *p){
    if(h == NULL) return 0;
    int pos = chaveDobra(elem, h->tam);
    if(h->tabela[pos] == NULL) return 0;
    if(*(h->tabela[pos]) == elem){
...
int insereHash_EnderAberto(Hash* h, int elem){
  if(h == NULL) return 0;
  int i, pos, newPos;
